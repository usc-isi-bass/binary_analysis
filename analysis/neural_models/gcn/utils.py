...
def load_data_v2(graphs, labels):
    adj = []
    features = []
    nb_classes = len(np.unique(labels))
    one_hot_targets = np.eye(nb_classes)[labels]
    
    node_label_idx_dict = {"UNK":0}

    for g in graphs:
        adj.append(nx.adjacency_matrix(g))
        for l in nx.get_node_attributes(g, 'label'):
            if l in node_label_idx_dict:
                continue
            else:
                node_label_idx_dict[l] = len(node_label_idx)
    E = len(node_label_idx)
    for g in graphs:
        N = nx.number_of_nodes(g)
        node_labels = nx.get_node_attributes(g, 'label')
        node_label_idx = [node_label_idx_dict.get(l, "UNK") for l in node_labels]
        one_hot_feature = np.eye(E)[node_label_idx]
        features.append(one_hot_feature)
    
    train_idx, test_and_val_idx, y_train, y_test_and_val  = train_test_split(np.arange(len(graphs)), one_hot_targets, test_size=0.2)
    test_idx, val_idx, y_test, y_val = train_test_split(test_and_val_idx, y_test_and_val, test_size=0.5)
    
    train_mask = sample_mask(train_idx, targets.shape[0])
    test_mask = sample_mask(test_idx, targets.shape[0])
    val_mask = sample_mask(val_idx, targets.shape[0])
